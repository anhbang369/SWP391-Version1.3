...
public class DAOIssue {
    ...
    private static final String UPDATE_PRODUCT = "UPDATE Product SET quantity=? WHERE productID=?";
    ...
    private static final String SEARCH_ISSUE_FULL = "SELECT i.issueID,i.note,i.accountantID,i.sellerID,i.DateP,od.orderDetailID,od.quantity,od.productID FROM Issue AS i,Orders AS o,OrderDetail AS od WHERE i.orderID=o.orderID AND o.orderID=od.orderID AND i.orderID=?";
    ...
    public boolean updateQuantityProduct(UserProductU user) throws SQLException {
        boolean check = false;
        Connection conn = null;
        PreparedStatement ptm = null;
        try {
        ...
        try {
            ...
            if (con != null) {
                ...
                while (rs.next()) {
                    ...
                    int quantity = Integer.parseInt(rs.getString("quantity"));
                    String productID = rs.getString("productID");
                    listIssue.add(new UserIssueFull(issueID, note, accountantID, sellerID, DateP, orderDetailID, quantity, productID));
